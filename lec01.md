...
@quizQuestion(В чем отличия машинного обучения от математической статистики?)

<!-- data-randomize -->
[[ ]] Отличий нет, это одно и то же
[[x]] В математической статистике, как правило, рассматриваются более интерпретируемые модели
[[x]] В машинном обучении, как правило, параметры моделей получаются в результате итерационной процедуры обучения, а в математической статистике – в результате аналитических расчетов
[[ ]] Для построения моделей машинного обучения необходима база данных
...
@quizQuestion(Соотнесите модели:)
...
@quizQuestion(`Что изучает машинное обучение?`)
...
@quizQuestion(`Какой подход к моделированию основан на известных законах и правилах?`)
...
@quizQuestion(`Какой этап пайплайна машинного обучения включает предобработку данных?`)
...
@quizQuestion(`Сопоставьте задачи с подходами к моделированию:`)
...
@quizQuestion(`Функция выделения признаков для тестовых данных *должна* совпадать с функцией выделения признаков для обучающих данных?`)

[(x)] да
[( )] нет


@quizQuestion(`В чем ключевое различие между Supervised и Unsupervised Learning?`)
...
@quizQuestion(`Представь, что ты агент в задаче обучения с подкреплением. Какое действие ты бы выбрал, чтобы получить долговременный выигрыш?`)

[( )] Пойти пить пиво🍺
[(x)] Посмотреть лекцию по машинному обучению👨‍💻


@quizQuestion(`Выберите верные варианты`)
...
@quizQuestion(`Почему глубокое обучение так популярно в последние годы?`)
...
@quizQuestion(`Что оказывает наибольшее влияние на качество обученной модели?`)
...
@quizQuestion(`В каких случаях глубокое обучение требует наличия размеченных данных (меток)?`)
